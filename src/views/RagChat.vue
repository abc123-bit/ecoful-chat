...
async function send() {
  ...
  let renderQueue = '';
  let draining = false;
  const questionSnippet = question.length > 20 ? `${question.slice(0, 20)}...` : question
  const nowIso = new Date().toISOString()

  if (isNewConversation) {
    // 侧边栏没有历史会话时，预先插入一条占位会话便于立即渲染
    pendingConversationId = `pending-${Date.now()}`
    conversations.value.unshift({
      session_id: pendingConversationId,
      id: pendingConversationId,
      title: questionSnippet,
      created_at: nowIso,
      updated_at: nowIso,
      pending: true
    })
    conversations.value = [...conversations.value]
    state.conversationId = pendingConversationId
  } else {
  ...
  const payload = {
    ...
    question,
    conversation_id: isNewConversation ? null : state.conversationId,
      ...
      (evt) => {
        if (evt?.type === 'start') {
          if (evt.conversation_id) {
            // 首次收到 start 事件时同步会话列表，确保侧边栏即时展示新对话
            const newId = evt.conversation_id
            state.conversationId = newId
            const targetId = pendingConversationId && pendingConversationId !== newId ? pendingConversationId : newId
            let idx = conversations.value.findIndex(item => (item.session_id || item.id) === targetId)
            if (idx === -1 && pendingConversationId && pendingConversationId !== newId) {
              idx = conversations.value.findIndex(item => (item.session_id || item.id) === newId)
            }
            ...
            if (idx === -1) {
              conversations.value.unshift({
                ...
                id: newId,
                title: questionSnippet,
                created_at: nowIsoStart,
                updated_at: nowIsoStart
              })
            } else {
              const current = conversations.value[idx]
              current.session_id = newId
              current.id = newId
              current.pending = false
              current.updated_at = nowIsoStart
              if (!current.title) current.title = questionSnippet
              if (idx > 0) {
          ...
          if (state.conversationId) {
            const idx = conversations.value.findIndex(item => (item.session_id || item.id) === state.conversationId)
            if (idx !== -1) {
              // 更新会话的时间戳以触发 UI 刷新
